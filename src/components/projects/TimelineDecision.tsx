...
/** RL POC approve/reject a specific pending project-timeline proposal. */
  ...
  async function decide() {
    ...
    try {
      await apiFetch(`/api/projects/${projectId}/timeline`, {
        method: "PATCH",
        body: JSON.stringify({ proposalId, action, decisionComment: comment || undefined }),
      });
      toast.success(action === "approve" ? "Timeline approved" : "Timeline rejected");
      setAction(null);
      setComment("");
      router.refresh();
    } catch (e) {
  ...
  return (
    <>
      ...
      <Dialog open={action !== null} onOpenChange={(o) => !o && setAction(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{action === "approve" ? "Approve timeline" : "Reject timeline"}</DialogTitle>
          </DialogHeader>
          <Field
            ...
            hint={
              action === "approve"
                ? "Approving applies the proposed dates to the project timeline."
                : "The PM will need to propose a revised timeline."
            }
          >
